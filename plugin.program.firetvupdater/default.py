...
def index():
	if AUTOUPDATE == 'Yes':
		if wiz.workingURL(WIZARDFILE) == True:
			...
			if ver > VERSION: addFile('%s [v%s] [COLOR red][UPDATE v%s][/COLOR]' % (ADDONTITLE, VERSION, ver), 'wizardupdate', themeit=THEME2)
	...
	if len(BUILDNAME) > 0:
		...
		if version > BUILDVERSION: build = '%s [COLOR red][UPDATE v%s][/COLOR]' % (build, version)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		...
		if len(match) == 1:
			...
			addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME2)
			...
			if SEPERATE == 'true':
				...
				if count17 > 0:
					...
					addFile('%s Krypton Builds(%s)' % (state, count16), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW17 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
				...
				if count16 > 0:
					...
					addFile('%s Jarvis Builds(%s)' % (state, count16), 'togglesetting',  'show16', themeit=THEME3)
					if SHOW16 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							kodiv = int(float(kodi))
				...
				if count15 > 0:
					...
					addFile('%s Isengard and Below Builds(%s)' % (state, count15), 'togglesetting',  'show15', themeit=THEME3)
					if SHOW15 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							kodiv = int(float(kodi))
		...
		if not themefile == 'http://':
			if wiz.workingURL(themefile) == True:
				...
				for themename, themeurl, themeicon, themefanart, description in match:
					...
					addFile(themename if not themename == BUILDTHEME else "%s (Installed)" % themename, 'theme', name, themename, description=description, fanart=themefanart, icon=themeicon, themeit=THEME3)
	setView('files', 'viewType')
			...
			if len(match) > 0:
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to remove and reinstall:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="Yes Remove", nolabel="No Skip"):
							wiz.cleanHouse(os.path.join(ADDONS, plugin))
							if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to remove the addon_data for:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="Yes Remove", nolabel="No Skip"):
					...
					if not repository.lower() == 'none' and not os.path.exists(repo):
						...
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to install the repository for [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="Yes Install", nolabel="No Skip"): 
...
def maintMenu(view=None):
	...
	addDir ('Cleaning Tools'       ,'maint', 'clean',  icon=ICONMAINT, themeit=THEME1)
	if view == "clean" or SHOWMAINT == 'true': 
		addFile('Total Clean Up: [COLOR green]%s[/COLOR]' % wiz.convertSize(totalsize)  ,'fullclean',       icon=ICONMAINT, themeit=THEME3)
		addFile('Clear Cache: [COLOR green]%s[/COLOR]' % wiz.convertSize(sizecache)     ,'clearcache',      icon=ICONMAINT, themeit=THEME3)
		addFile('Clear Packages: [COLOR green]%s[/COLOR]' % wiz.convertSize(sizepack)   ,'clearpackages',   icon=ICONMAINT, themeit=THEME3)
		addFile('Clear Thumbnails: [COLOR green]%s[/COLOR]' % wiz.convertSize(sizethumb),'clearthumb',      icon=ICONMAINT, themeit=THEME3)
	...
	addDir ('Addon Tools',       'maint', 'addon',  icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('Misc Maintenance'     ,'maint', 'misc',   icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('Back up/Restore'     ,'maint', 'backup',   icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('System Tweaks/Fixes',       'maint', 'tweaks', icon=ICONMAINT, themeit=THEME1)
	...
	if autoclean == 'true':
		addFile('--- Auto Clean Fequency: [COLOR green]%s[/COLOR]' % feq[AUTOFEQ], 'changefeq', icon=ICONMAINT, themeit=THEME3)
...
def writeAdvanced(name):
	if not ADVANCEDFILE == 'http://':
		...
		if ADVANCEDWORKING == True:
			...
			if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to overwrite your current Advanced Settings with [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="Overwrite", nolabel="Cancel")
			else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to download and install [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="Install", nolabel="Cancel")
...
def buildWizard(name, type, theme=None):
	...
	if type == 'gui':
		if name == BUILDNAME:
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to apply the guifix for:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='No, Cancel',yeslabel='Apply Fix')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]WARNING!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] community build is not currently installed." % (COLOR2, COLOR1, name), "Would you like to apply the guiFix anyways?.[/COLOR]", nolabel='No, Cancel',yeslabel='Apply Fix')
		if yes:
			...
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s]Downloading GuiFix:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Please Wait')
			lib=os.path.join(PACKAGES, '%s_guisettings.zip' % zipname)
			try: os.remove(lib)
			except: pass
			downloader.download(buildzip, lib, DP)
			xbmc.sleep(500)
			title = '[COLOR %s]Installing:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name)
			DP.update(0, title,'', 'Please Wait')
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Do you wish to restore your" % COLOR2, "Kodi configuration to default settings", "Before installing the local backup?[/COLOR]", nolabel='No, Cancel', yeslabel='Continue')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Do you wish to restore your" % COLOR2, "Kodi configuration to default settings", "Before installing [COLOR %s]%s[/COLOR]?" % (COLOR1, install), nolabel='No, Cancel', yeslabel='Continue')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Do you wish to restore your" % COLOR2, "Kodi configuration to default settings?[/COLOR]", nolabel='No, Cancel', yeslabel='Continue')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
			yes=DIALOG.yesno(ADDONTITLE, "[COLOR %s]The skin needs to be set back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, skin[5:]), "Before doing a fresh install to clear all Texture files,", "Would you like us to do that for you?[/COLOR]", yeslabel="Yes Switch It", nolabel="I'll Do It");
			if yes:
				skinSwitch.swapSkins(skin)
				x = 0
				xbmc.sleep(1000)
