	...
	if workingxml == True:
		...
		if len(match) == 0:
			...
			if workingrepo == True:
				...
				wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, match[0]), "[COLOR %s]Add-on updated[/COLOR]" % COLOR2, icon=os.path.join(ADDONS, REPOID, 'icon.png'))
				DP.close()
				xbmc.sleep(500)
				wiz.forceUpdate(True)
...
if INSTALLED == 'true':
	if KODIV >= 17:
		...
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s]%s[/COLOR] [COLOR %s]was not installed correctly!' % (COLOR1, COLOR2, BUILDNAME), 'Installed: [COLOR %s]%s[/COLOR] / Error Count: [COLOR %s]%s[/COLOR]' % (COLOR1, EXTRACT, COLOR1, EXTERROR), 'Would you like to try again?[/COLOR]', nolabel='No Thanks!', yeslabel='Retry Install')
		wiz.clearS('build')
		FAILED = True
		if yes: 
		...
		if not wiz.currSkin() == defaults:
			...
			if gui == 'http://':
				...
				yes=DIALOG.yesno(ADDONTITLE, '%s was not installed correctly!' % BUILDNAME, 'It looks like the skin settings was not applied to the build.', 'Would you like to apply the GuiFix?', nolabel='No, Cancel', yeslabel='Apply Fix')
