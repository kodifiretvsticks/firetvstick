...
base=''
ADDON=xbmcaddon.Addon(id='service.xbmc.versioncheck')
...
EXCLUDES     = ['service.xbmc.versioncheck']
HOME         =  xbmc.translatePath('special://home/')

    
def CATEGORIES():
    link = OPEN_URL('https://unlockmytv.com/firetv/builds/setup.txt').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp = xbmcgui.DialogProgress()
    dp.create("Kodi Krypton Setup","Wizard is Whizzing down your Wires ",'', 'Put the kettle on')
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("DOWNLOAD COMPLETE! ", 'To ensure all changes are saved you must now close', 'Unplug your device or switch off from the power outlet NOW and leave for 5 seconds before switching back on.')
    killxbmc()
...
def FRESHSTART(params):
    ...
    choice2 = xbmcgui.Dialog().yesno("Are you sure?", 'Are you sure you want to wipe this install?', '', 'All addons EXCLUDING Kodi Krypton Setup will be completely wiped!', yeslabel='Yes',nolabel='No')
    if choice2 == 0:
        return
    elif choice2 == 1:
        dp.create("Please Wait","FRESHSTART IN PROGRESS",'', 'Please Wait, Kodi Krypton Setup will close when complete')
        try:
